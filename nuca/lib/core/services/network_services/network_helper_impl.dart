...
class NetworkHelperImpl extends NetworkHelper {
  ...
  @override
  Future<http.Response> delete(String url, {body, Map? headers}) async {
    final header = await appendHeader(headers: headers);
    return http
  ...
  Future<Map<String, String>> appendHeader({
    ...
    try {
      ...
      debugPrint("Bearer ${sharedPreferences.get(StorageKeys.token)}");
    } catch (e) {
  ...
  Future<http.Response> postMultipartData(
    ...
    for (int i = 0; i < multipartBody.length; i++) {
      var file = await http.MultipartFile.fromPath(
        ...
        contentType: getMediaType(multipartBody[i].name),
      );
  ...
  Future<http.Response> postMultipartLiscenceData(
    ...
    var file = await http.MultipartFile.fromPath(
      'image',
      multipartBody.path,
      contentType: getMediaType(multipartBody.name),
    );
    request.files.add(file);
    http.Response response = await http.Response.fromStream(
      await request.send(),
